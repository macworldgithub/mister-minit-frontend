...
export const API_CONFIG = {
  baseURL: (import.meta as { env?: Record<string, string> }).env?.VITE_API_URL || 'http://localhost:3005',
  // Allow toggling via VITE_USE_MOCK in env or window.localStorage('USE_MOCK')
  useMock:
    typeof window !== 'undefined' && window.localStorage?.getItem('USE_MOCK') !== null
      ? window.localStorage.getItem('USE_MOCK') === 'true'
      : (import.meta as { env?: Record<string, string> }).env?.VITE_USE_MOCK === 'true' ? true : false,
};
