...
export const smsThreadService = {
  async getThreads(filters: ThreadFilters = {}): Promise<SmsThread[]> {
    if (API_CONFIG.useMock) {
      ...
      return mockThreads.filter((thread) => {
        if (filters.storeId && filters.storeId !== 'all' && thread.storeId !== filters.storeId) {
          return false;
        }
        ...
        if (filters.search) {
          ...
          const matchStore = thread.storeName?.toLowerCase().includes(q) ?? false;
          const matchCustomer = thread.bookingDetails?.customerName?.toLowerCase().includes(q) ?? false;
          const matchContent = thread.conversationHistory.some((c) => c.content.toLowerCase().includes(q));
    ...
    return request<SmsThread[]>(`/sms-threads?${params.toString()}`);
  },
  ...
  async updateThreadStatus(threadId: string, status: string, closedReason?: string): Promise<SmsThread> {
    if (API_CONFIG.useMock) {
  ...
  async appendMessage(threadId: string, content: string, role: 'user' | 'assistant' = 'assistant'): Promise<SmsThread> {
    if (API_CONFIG.useMock) {
      mockThreads = mockThreads.map((t) => {
        if (t._id === threadId) {
