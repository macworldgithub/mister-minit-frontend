...
interface StoresViewProps {
  ...
  onToggleActive: (did: string, currentActive?: boolean) => Promise<void> | void;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            <h3 className="text-base font-bold text-white">Pilot Store Directory ({stores.length})</h3>
            {onRefresh && (
              <button
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Stores participating in the automated 3CX missed-call SMS concierge pilot program
          </p>
        </div>
        <button
          onClick={handleAdd}
          className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-semibold flex items-center gap-2 transition-all shadow-lg shadow-red-900/30 cursor-pointer w-fit"
        >
      ...
      {(isLoading || isRefreshing) && stores.length === 0 && (
        <div className="bg-slate-900/50 border border-slate-800/60 rounded-xl p-12 text-center">
          <RefreshCw className="w-10 h-10 text-red-400 mx-auto mb-3 animate-spin" />
          <h4 className="text-sm font-bold text-white mb-1">Loading Store Configurations...</h4>
      ...
      {!isLoading && !isRefreshing && stores.length === 0 && (
        <div className="bg-slate-900/50 border border-slate-800/60 rounded-xl p-12 text-center">
          ...
          <h4 className="text-sm font-bold text-white mb-1">No Store Configs Found</h4>
          <p className="text-xs text-slate-400 mb-4 max-w-md mx-auto">
            No pilot store configurations found. Click below to add a new pilot store with DID.
          </p>
          <button
            onClick={handleAdd}
            className="px-4 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-white text-xs font-semibold inline-flex items-center gap-2 transition-colors cursor-pointer"
          >
              ...
              <div>
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-slate-800/60 border border-slate-700/60 flex items-center justify-center text-red-400">
                      <Store className="w-5 h-5" />
                    </div>
                    <div>
                      <h4 className="text-sm font-bold text-white tracking-tight">{store.storeName}</h4>
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-xs font-mono font-medium text-red-400 bg-red-500/10 border border-red-500/20 px-1.5 py-0.2 rounded">
                          DID: {store.did}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {onDeleteStore && (
                      <button
                        onClick={() => {
                          if (window.confirm(`Are you sure you want to delete "${store.storeName}" (${store.did})?`)) {
                ...
                <div className="flex items-start gap-2 text-xs text-slate-400 mb-3 bg-slate-950/60 p-2.5 rounded-xl border border-slate-800/60">
                  ...
                  <span className="leading-snug text-[11px]">{store.tradingHours}</span>
                </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {store.staffContacts.map((contact, idx) => (
                        ...
                        >
                          ...
                          <span className="text-slate-400 font-mono">({contact.mobile})</span>
                          {contact.email && (
                            <span className="text-slate-500 text-[10px] hidden sm:inline">· {contact.email}</span>
                          )}
                        </div>
                      ))}
                    </div>
              ...
              <div className="pt-3 border-t border-slate-800/60 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  >
                    {store.isActive ? (
                      <>
                        <CheckCircle2 className="w-3.5 h-3.5 text-red-400" /> Pilot Active
                      </>
                    ) : (
                      <>
                  ...
                  <span className="text-[11px] text-slate-500">
                    {store.isActive ? 'Concierge SMS enabled' : 'Calls will be suppressed'}
                  </span>
                </div>
