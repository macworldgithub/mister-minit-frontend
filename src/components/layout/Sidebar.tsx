...
export type NavTab = 'overview' | 'conversations' | 'stores' | 'cdr' | 'suppressed';
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const navItems: Array<{ id: NavTab; label: string; icon: React.ElementType; badge?: number }> = [
    ...
    { id: 'conversations', label: 'SMS Concierge', icon: MessageSquareText, badge: unreadCount },
  ...
  return (
    <>
      ...
      <aside
        className={`fixed lg:static top-0 bottom-0 left-0 z-50 w-64 bg-slate-900/95 lg:bg-slate-900/50 backdrop-blur-md border-r border-slate-800/60 flex flex-col flex-shrink-0 min-h-screen transition-transform duration-300 ease-in-out ${isOpenMobile ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'
          }`}
      >
        {/* Brand Header */}
        <div className="p-5 border-b border-slate-800/60 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-red-600 to-red-700 flex items-center justify-center shadow-lg shadow-red-900/30 text-white font-bold text-lg tracking-wider border border-red-500/30">
              MM
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <h1 className="font-semibold text-white text-base tracking-tight leading-none">Mister Minit</h1>
              ...
              <p className="text-xs text-slate-400 mt-1">SMS Concierge System</p>
            </div>
          </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-150 cursor-pointer text-left ${isActive
              ...
              >
                ...
                {item.badge && item.badge > 0 ? (
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full font-semibold ${isActive ? 'bg-red-500 text-white' : 'bg-slate-800 text-slate-300'
                      }`}
                  >
                    {item.badge}
                  </span>
